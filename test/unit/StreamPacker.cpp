...
TEST(StreamPacker, stream_pack_unpack) {
    std::array<uint8_t, 12> test_data{127, 255, 100, 33, 66, 111, 
        0, 75, 150, 120, 160, 220};
    ...
    test_stream.write(reinterpret_cast<char*>(test_data.data()), test_data.size());
    ...
    auto stream_unpacker = StreamUnpacker<ColorType>(std::move(test_stream_ptr),
            std::move(unpacker));
    ...
    auto stream_packer = StreamPacker<ColorType>(std::move(pack_stream),
            std::move(packer));

    stream_packer.pack(unpacked_colors_single.begin(), unpacked_colors_single.end());
    stream_packer << ColorType(55, 110, 235);

    pack_stream = std::unique_ptr<std::stringstream>(
        static_cast<std::stringstream*>(stream_packer.release_stream().release()));
    ...
    for(int i = test_data.size(); i < packed_string.size(); ++i) {
        ...
        ASSERT_EQ(static_cast<uint8_t>(packed_string[i]), end_test_data[scaled_i]); 
    }
}
