...
inline void for_each_channel_impl(TupleType&& channels, const FnType& fn) {
...
inline constexpr typename Color::ElementType get(const Color& color) {
...
inline void for_each_element(const Color& color, const FnType& fn) {
...
template <typename Color,
        typename std::enable_if_t<
                std::is_floating_point<typename Color::ElementType>::value,
                int> = 0>
inline bool float_eq(const Color& lhs,
        const Color& rhs,
        typename Color::ElementType threshold =
                FLOAT_EPSILON<typename Color::ElementType>) {
    auto is_equal = true;
    for_each_element(lhs,
            [&is_equal, &rhs, threshold](typename Color::ElementType elem,
                             std::size_t idx) {
